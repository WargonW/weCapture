...
| P4 | M14-M15: 增强功能（窗口截图 / 滚动长截图） | 待开发 |
| P5 | M16-M18: 完善（标注独立 / 平台抽象 / 成品化） | 待开发 |
...
**M14 应用窗口截图**
- 14.1 窗口列表采集：`list_windows()` 返回窗口标题 / 进程 / 矩形 / 最大化，新增 `platform/window_trait.rs`
- 14.2 悬浮窗口列表：选区阶段左下角浮层，点击自动吸附选区到窗口矩形
- 14.3 一键窗口截图：复用 `captureRegion` + 虚拟桌面坐标定位所属显示器
- 14.4 工作台新增「窗口截图」模块 + 快捷键 Ctrl+Shift+W

**M15 滚动长截图**
- 15.1 选区阶段「滚动模式」：区域 + 方向 + 滚轮速度
- 15.2 自动采集：`capture_region` → 模拟滚轮 → 间隔重抓，内容不变或达上限即停止
- 15.3 重叠拼接：行级差异匹配相邻帧公共行，去除重叠后纵向拼接
- 15.4 长图进入既有结果页（标注 / 保存 / 复制 / 贴图）

**P5 完善（M16–M18）**

**M16 标注系统独立化**
- 16.1 `core/annotation.rs`：统一标注模型 + serde
- 16.2 `annotation_service.rs`：前端传标注列表，Rust imageproc 绘制合成（替换前端 Canvas）
- 16.3 扩展箭头 / 矩形 / 荧光笔标注类型
- 16.4 几何测量标注（长度 / 角度）

**M17 平台抽象层落地**
- 17.1 定义 capture / recorder / hotkey 三个 trait
- 17.2 现有服务改为实现 trait，服务层只依赖接口
- 17.3 mockall mock 实现，无屏环境可单测
- 17.4 feature 开关平滑替换帧采集后端，默认保持 xcap

**M18 设置中心 + 成品化**
- 18.1 补 `settings_cmd.rs`：配置读写 JSON 持久化
- 18.2 SettingsView 扩展 Tab：常规（保存目录 / 默认格式 / DPI）、快捷键、关于
- 18.3 结果页：打开所在文件夹、MP4 / GIF 预览
- 18.4 打包发布：图标 / 产物名、tauri build 安装包、GitHub Actions CI
- 18.5 README 完善 + 交付核对
